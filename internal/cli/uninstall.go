...
func runUninstallCLI(args []string) {
	...
	typ := fs.String("type", "", "Transport: reality|vless|hysteria|wireguard|wss|tls (derives service name)")
	...
	if serviceName == "" {
		fmt.Fprintln(os.Stderr, "uninstall: specify --service NAME or --type reality|vless|hysteria|wireguard|wss|tls")
		os.Exit(2)
	}
